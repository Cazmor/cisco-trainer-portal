...
router.put('/notifications', async (req, res) => {
    try {
        const { email_reminders, weekly_report_reminder, maintenance_alerts, uaf_expiry_reminders, student_at_risk_alerts, report_emails, reminder_time } = req.body;
        const result = await query("UPDATE notification_preferences SET email_reminders = COALESCE($1, email_reminders), weekly_report_reminder = COALESCE($2, weekly_report_reminder), maintenance_alerts = COALESCE($3, maintenance_alerts), uaf_expiry_reminders = COALESCE($4, uaf_expiry_reminders), student_at_risk_alerts = COALESCE($5, student_at_risk_alerts), report_emails = COALESCE($6, report_emails), reminder_time = COALESCE($7, reminder_time) WHERE user_id = $8 RETURNING *", [email_reminders, weekly_report_reminder, maintenance_alerts, uaf_expiry_reminders, student_at_risk_alerts, report_emails, reminder_time, req.user.id]);
        if (result.rows.length === 0) { const insertResult = await query("INSERT INTO notification_preferences (user_id, email_reminders, weekly_report_reminder, maintenance_alerts, uaf_expiry_reminders, student_at_risk_alerts, report_emails, reminder_time) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *", [req.user.id, email_reminders, weekly_report_reminder, maintenance_alerts, uaf_expiry_reminders, student_at_risk_alerts, report_emails, reminder_time]); return res.json(insertResult.rows[0]); }
        res.json(result.rows[0]);
    } catch (error) { res.status(500).json({ error: 'Internal server error' }); }
});
